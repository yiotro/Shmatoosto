...
public abstract class InterfaceElement<T extends InterfaceElement<T>> {
    ...
    public InterfaceElement(MenuControllerYio menuControllerYio, int id) {
        ...
        setParent(screen);
        setAnimation(AnimationYio.def); // after setParent()
    }
    ...
    protected void updateViewPosition() {
        factor = appearFactor.get();

        if (animType == AnimationYio.none) {
        ...
        viewPosition.set(x1, y1, 2 * hor, 2 * ver);
    }


    private void animRight() {
        x1 = position.x + (1 - factor) * (GraphicsYio.width - position.x);
        hor = 0.5f * position.width;
        ver = 0.5f * position.height;
        y1 = position.y;
    }


    private void animHorizontalFromCenter() {
        ...
        cx += (1 - appearFactor.get()) * (GraphicsYio.width / 2 - cx);
    ...
    protected void animDownShort() {
        ...
        hor = 0.5f * position.width;
        ver = 0.5f * position.height;
        y1 = position.y - (1 - factor) * animDistance;
    }
    ...
    protected void animDown() {
        ...
        hor = 0.5f * position.width;
        ver = 0.5f * position.height;
        y1 = factor * (position.y + position.height) - position.height;
    }
    ...
    protected void animUp() {
        ...
        hor = 0.5f * position.width;
        ver = 0.5f * position.height;
        y1 = position.y + ((1 - factor) * (menuControllerYio.yioGdxGame.h - position.y));
    }
